...
export function ServiceCard() {
  ...
  {cardData.map((card, index) => (
    <Link
      key={index}
      href={`https://www.saynopest.com/services/${card.category}`}
      className="block" // make sure the whole card is clickable
    >
      <div className="relative flex w-full max-w-sm flex-col rounded-xl bg-white bg-clip-border text-gray-700 shadow-lg mx-4 hover:shadow-xl transition-shadow duration-300">
        ...
        <div className="p-6">
          ...
          <div className="flex flex-row justify-center p-3">
            <button className="block mx-1 w-full px-20 rounded-lg bg-gray-900 py-3.5 text-sm font-bold uppercase text-white shadow-md hover:shadow-lg transition-all">
              Know More
            </button>
          </div>
        </div>
